...
const selectedUserRole = computed(() => {
    ...
    if (!user) return ''
    return user.role === 'ROOT' ? 'SUPERUSUARIO (ROOT)' : 'ADMINISTRADOR DEL SISTEMA'
})
            ...
            <div class="bg-white/95 backdrop-blur-2xl rounded-3xl shadow-[0_32px_64px_-16px_rgba(0,0,0,0.5)] overflow-hidden border border-white/20">
                <div class="p-8">
                    ...
                    <form @submit.prevent="handleLogin" class="space-y-6">
                        <!-- Selector de Usuario -->
                        <div class="space-y-2">
                            <label class="block text-xs font-black text-emerald-900 uppercase tracking-wider ml-1">Personal Autorizado</label>
                        ...
                        <!-- Password -->
                        <div class="space-y-2">
                            <label class="block text-xs font-black text-emerald-900 uppercase tracking-wider ml-1">Contraseña de Seguridad</label>
                            <div class="relative group">
                                <input v-model="password" type="password" required placeholder="••••••••"
                                    class="w-full pl-12 pr-4 py-4 bg-gray-50 border-2 border-gray-100 rounded-2xl text-gray-800 font-bold focus:border-emerald-500 focus:bg-white outline-none transition-all">
                                <div class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-emerald-500 transition-colors">
                        ...
                        <p v-if="error" class="text-center text-red-500 font-bold text-sm animate-bounce">{{ error }}</p>
                    </form>
                </div>
            </div>
